...
const cards = [
  ...
  c("D3.5.8", "S3 Glacier", "What are the Amazon S3 Glacier storage classes?", "<strong>S3 Glacier Instant Retrieval</strong> is for rarely accessed data that still needs millisecond retrieval. <strong>S3 Glacier Flexible Retrieval</strong> retrieves data in minutes to hours at lower cost. <strong>S3 Glacier Deep Archive</strong> is the lowest-cost S3 class, designed for data retained for 7 to 10 years with retrieval in 12 hours or less.", "Like archiving old tax records in a warehouse — cheaper the further back they go, but slower to retrieve.", "Glacier classes are S3 storage tiers, not separate services. Do not confuse with AWS Backup.", "Glacier Instant = ms retrieval. Flexible = minutes to hours. Deep Archive = 12-hour retrieval, lowest cost."),
  ...
  c("D4.3.5", "Trusted Advisor", "What is AWS Trusted Advisor?", "<strong>AWS Trusted Advisor</strong> inspects your AWS environment and provides real-time recommendations across five categories: <strong>Cost Optimization</strong>, <strong>Performance</strong>, <strong>Security</strong>, <strong>Fault Tolerance</strong>, and <strong>Service Limits</strong>.", "Like a car inspection that checks safety, fuel efficiency, and wear across five categories.", "Basic and Developer support plans receive only a core subset of Trusted Advisor checks. Business and Enterprise support get the full set.", "Five Trusted Advisor categories: Cost, Performance, Security, Fault Tolerance, Service Limits."),
  ...
  c("D4.6.1", "Basic Support", "What is AWS Basic Support?", "<strong>Basic Support</strong> is included for all AWS customers and provides account and billing support, documentation, service health info, and a limited set of Trusted Advisor checks.", "", "", "Free for all AWS accounts = Basic Support."),
  c("D4.6.4", "Developer Support", "What is AWS Developer Support?", "<strong>AWS Developer Support</strong> adds email access to Cloud Support Associates during business hours. General guidance is answered within 24 business hours and system impairments within 12 business hours. It is suited for <strong>development and testing</strong> environments.", "", "Developer Support does not include 24/7 phone or chat access to engineers — that requires Business support or higher.", "Dev and test workloads, email-only access = Developer Support."),
...
function init() {
  const savedTheme = localStorage.getItem("awsFlashcardTheme") || "light";
...
function renderCard(direction) {
  triggerAnimation(els.flashcard, direction === "prev" ? "nav-prev" : direction === "shuffle" ? "nav-shuffle" : "nav-next");
  triggerAnimation(els.cardCounter, "pop");
  ...
  if (!state.filtered.length) {
    ...
    els.cardStatusBadge.className = "status-badge hidden";
    return;
  }
  ...
  els.cardExtras.innerHTML = [
    ...
    card.trap    ? `<div class="extra trap"><strong>Exam trap</strong> ${card.trap}</div>` : "",
  ...
  els.knownBtn.textContent  = status === "known"  ? "Known (marked)"        : "Known";
  els.reviewBtn.textContent = status === "review" ? "Needs review (marked)" : "Needs review";

  if (status) {
    els.cardStatusBadge.textContent = status === "known" ? "Known" : "Review";
    els.cardStatusBadge.className = `status-badge ${status}`;
  } else {
    els.cardStatusBadge.className = "status-badge hidden";
  }
}
...
function triggerAnimation(el, cls) {
  el.classList.remove("nav-next", "nav-prev", "nav-shuffle", "pop");
  void el.offsetWidth;
...
function mark(status) {
  ...
  updateProgress();
  showToast(status === "known" ? "Marked as known" : "Marked for review", status);
  nextCard();
}
...
function renderPairs() {
  els.pairGrid.innerHTML = confusedPairs.map((item, idx) => {
    const [leftLabel, rightLabel] = item.title.split(" vs ");
    return `
      <article class="pair-card" style="animation-delay:${idx * 0.055}s">
        <h3>${item.title}</h3>
        <table>
          <thead>
            <tr><th>${leftLabel || "First"}</th><th>${rightLabel || "Second"}</th></tr>
          </thead>
          <tbody>
            ${item.left.map((left, i) => `<tr><td>${left}</td><td>${item.right[i] || ""}</td></tr>`).join("")}
          </tbody>
        </table>
      </article>
    `;
...
function toggleDarkMode() {
  ...
  localStorage.setItem("awsFlashcardTheme", next);
